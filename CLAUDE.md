...
## Рабочий процесс: mattpocock/skills

Используем систематический подход к разработке на базе [mattpocock/skills](https://github.com/mattpocock/skills):

### Перед реализацией
- **/grilling** — интервьюирование идеи, уточнение требований, выявление edge-cases
- **/deep-research** — глубокое исследование при работе с нормативикой (СП, ГОСТ)

### Во время разработки
- **/tdd** — разработка через тестирование для Core-логики (особенно алгоритмов раскладки)
- **/prototype** — быстрое прототипирование UI и интеграции с Revit

### После реализации
- **/code-review** — рецензия изменений по Standards (соответствие архитектурному правилу, идиомы C#/.NET) и Spec (соответствие требованиям)
- **/simplify** — упрощение и оптимизация кода

### Принципы
1. **Глубокое понимание перед кодом** — не начинаем реализацию без ясного образа решения.
2. **Тесты как спецификация** — в Core тесты пишутся перед кодом (или одновременно).
3. **Разделение ответственности** — архитектурное правило (Core ≠ Revit API) проверяется при каждой code-review.
4. **Рецензия по двум осям** — Standards (как правильно) и Spec (что нужно).
